...
const API_END_POINT = "https://foody-6cr4.onrender.com/api/v1/restaurant";
...
export const useRestaurantStore = create<RestaurantState>()(persist((set, get) => ({
    ...
    searchRestaurant: async (searchText: string, searchQuery: string, selectedCuisines: any) => {
        try {
            ...
            //dalay api call for showing loading skeleton
    ...
    updateMenuToRestaurant: (updatedMenu: MenuItem) => {
        set((state: any) => {
            if (state.restaurant) {
                ...
                return { restaurant: { ...state.restaurnat, menus: updatedMenuList } }
            }
            return state;
        });
    },
